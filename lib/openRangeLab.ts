...
//
// 【這一版做了什麼】
// - 開盤區間：09:30～10:00 美東時間（30分鐘），用瀏覽器原生 Intl.DateTimeFormat 搭配
//   America/New_York 時區換算，夏令時間(DST)由瀏覽器的時區資料庫自動處理，沒有寫死偏移量。
// - 突破判定：只做「收盤突破」（10:00～10:30 那根K棒收盤價 > 區間高 / < 區間低）。
// - 進場：突破確認後，下一根K棒（10:30 開盤）才進場，不用突破那根的收盤價假設成交。
...
// tpMultiple: 1、2 或 3（對應 1R/2R/3R）
export function runOpenRangeBacktest(symbol: string, candles30m: Candle[], tpMultiple: number): OpenRangeTrade[] {
  const trades: OpenRangeTrade[] = [];

  for (let i = 0; i < candles30m.length - 2; i++) {
    const rangeBar = candles30m[i];
    const info = getETInfo(rangeBar.time);
    if (info.hour !== 9 || info.minute !== 30 || !WEEKDAYS.includes(info.weekday)) continue;

    const signalBar = candles30m[i + 1];
    const entryBar = candles30m[i + 2];
    // 安全檢查：確保三根K棒是連續的30分鐘（沒有資料缺口）
    if (signalBar.time - rangeBar.time !== 1800 || entryBar.time - signalBar.time !== 1800) continue;

    const rangeHigh = rangeBar.high;
    const rangeLow = rangeBar.low;

    let direction: "LONG" | "SHORT" | null = null;
    if (signalBar.close > rangeHigh) direction = "LONG";
    else if (signalBar.close < rangeLow) direction = "SHORT";
    ...
    for (let j = i + 2; j < candles30m.length; j++) {
